...
res_dir <- here::here("results","self_test", "fit_1")
dir.create(res_dir)
...
resid_SSB_N <- sapply(sims[use], \(x) x$SSB[,1]/fits[[2]]$rep$SSB[,1] - 1)
med_res_SSB_N <- apply(resid_SSB_N, 1, median, na.rm = TRUE)
plot(fits[[2]]$years, med_res_SSB_N)
abline(h = 0)
abline(h = median(med_res_SSB_N, na.rm = TRUE), lty =2)
abline(h = median(resid_SSB_N, na.rm = TRUE), lty =2, col = "red")

resid_SSB_S <- sapply(sims[use], \(x) x$SSB[,2]/fits[[2]]$rep$SSB[,2] - 1)
med_res_SSB_S <- apply(resid_SSB_S, 1, median, na.rm = TRUE)
plot(fits[[2]]$years, med_res_SSB_S)
abline(h = 0)
abline(h = median(resid_SSB_S, na.rm = TRUE), lty =2)

sapply(sims, \(x) max(abs(x$grad)))

resid_par <- sapply(sims, \(x) x$par - fits[[2]]$opt$par)
med_res_par <- apply(resid_par, 1, median)
plot(med_res_par, xlab = "par")
abline(h = 0)

#seeds <- sample(0:1e9, 100, replace = TRUE)
res_dir <- here::here("results","self_test", "fit_1_fix")
dir.create(res_dir)
fit_1 <- readRDS(here::here("results","fit_1.RDS"))
...
reml_resid_SSB_S <- sapply(self_test_reml[[1]], \(x) x$SSB[,2]/fits[[2]]$rep$SSB[,2] - 1)
ml_resid_SSB_S <- sapply(self_test_ml[[1]], \(x) x$SSB[,2]/fits[[2]]$rep$SSB[,2] - 1)
med_reml_res_SSB_S <- apply(reml_resid_SSB_S, 1, median, na.rm = TRUE)
med_ml_res_SSB_S <- apply(ml_resid_SSB_S, 1, median, na.rm = TRUE)
plot(fits[[2]]$years, med_reml_res_SSB_S, ylim = range(c(med_reml_res_SSB_S, med_ml_res_SSB_S), na.rm =T), ylab = "Rel Diff.")
mtext(side = 3, "REML")
abline(h = 0)
...
plot(fits[[2]]$years, med_ml_res_SSB_S, ylim = range(c(med_reml_res_SSB_S, med_ml_res_SSB_S), na.rm =T), ylab = "")
mtext(side = 3, "ML")
abline(h = 0)
#median of annual medians (n_years)
...
par(mfrow = c(1,2))
reml_resid_SSB_N <- sapply(self_test_reml[[1]], \(x) x$SSB[,1]/fits[[2]]$rep$SSB[,1] - 1)
ml_resid_SSB_N <- sapply(self_test_ml[[1]], \(x) x$SSB[,1]/fits[[2]]$rep$SSB[,1] - 1)
med_reml_res_SSB_N <- apply(reml_resid_SSB_N, 1, median, na.rm = TRUE)
med_ml_res_SSB_N <- apply(ml_resid_SSB_N, 1, median, na.rm = TRUE)
plot(fits[[2]]$years, med_reml_res_SSB_N, ylim = range(c(med_reml_res_SSB_N, med_ml_res_SSB_N), na.rm =T), ylab = "Rel Diff.")
mtext(side = 3, "REML")
abline(h = 0)
#median of annual medians (n_years)
...
plot(fits[[2]]$years, med_ml_res_SSB_N, ylim = range(c(med_reml_res_SSB_N, med_ml_res_SSB_N), na.rm =T), ylab = "")
mtext(side = 3, "ML")
abline(h = 0)
#median of annual medians (n_years)
...
ml_resid_mu_SN <- sapply(self_test_ml[[1]], \(x) x$rep$mu[1,8,1,1,2,1] - fits[[2]]$rep$mu[1,8,1,1,2,1])
ml_resid_mu_NS <- sapply(self_test_ml[[1]], \(x) x$rep$mu[1,8,11,1,1,2] - fits[[2]]$rep$mu[1,8,11,1,1,2])
